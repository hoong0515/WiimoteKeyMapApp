  ...
  try {
    ...
    if (j.contains("mappings")) {
      ...
      for (auto &[key, val] : j["mappings"].items()) {
        ...
        if (val.is_array()) {
          for (auto &k : val) keys.push_back(StringToVK(k.get<std::string>()));
...
void ConfigManager::SaveActiveConfig() {
  ...
  for (const auto &[btn, keys] : m_mappings) {
    ...
    for (WORD vk : keys) keyArray.push_back(VKToString(vk));
...
void ConfigManager::SaveProfile(const std::string &profileName) {
  try {
    if (!fs::exists("profiles")) fs::create_directory("profiles");
  } catch (...) {}
  ...
  if (file.is_open()) {
    ...
    std::cout << "Exported current settings to profile: " << profileName << std::endl;
  }
}
...
WORD ConfigManager::StringToVK(const std::string &keyStr) const {
  ...
  if (keyStr == "L-Shift") return VK_LSHIFT;
  if (keyStr == "R-Shift") return VK_RSHIFT;
  if (keyStr == "L-Ctrl") return VK_LCONTROL;
  if (keyStr == "R-Ctrl") return VK_RCONTROL;
  if (keyStr == "L-Alt") return VK_LMENU;
  if (keyStr == "R-Alt") return VK_RMENU;
  if (keyStr == "Win") return VK_LWIN;
  if (keyStr == "Right Win") return VK_RWIN;
  if (keyStr == "Mouse Left") return VK_LBUTTON;
  if (keyStr == "Mouse Right") return VK_RBUTTON;
  if (keyStr == "Mouse Middle") return VK_MBUTTON;

  for (int i = 1; i < 256; ++i) {
    if (VKToString((WORD)i) == keyStr) return (WORD)i;
  }
  return 0;
}
...
std::string ConfigManager::VKToString(WORD vk) const {
  ...
  if (vk == VK_INSERT || vk == VK_DELETE || vk == VK_HOME || vk == VK_END ||
      ...
      vk == VK_RIGHT || vk == VK_DOWN || vk == VK_RCONTROL ||
      vk == VK_RMENU) {
    lParam |= (1 << 24);
  }

  if (GetKeyNameTextA(lParam, name, sizeof(name)) > 0) return std::string(name);
  char hex[16];
